...
class DeviceUpdate(BaseModel):
    ...
    model: str | None = None
    status: models.DeviceStatus | None = None
...
class LicenseUpdate(BaseModel):
    ...
    license_type: models.LicenseType | None = None
...
async def create_vendor(
    ...
    vendor = models.Vendor(vendor_id=str(uuid.uuid4()), **vendor_data.model_dump())
    db.add(vendor)
    db.commit()
    db.refresh(vendor)
    audit = AuditService(db)
...
async def update_vendor(
    ...
    vendor = (
        db.query(models.Vendor).filter(models.Vendor.vendor_id == vendor_id).first()
    )
    if not vendor:
...
async def validate_device_ip(
    request: dict, current_user: models.User = Depends(get_current_user)
):
    ...
    if is_valid:
        return {"valid": True, "message": f"Valid IP address: {ip_address}"}
    else:
        return {
            "valid": False,
...
async def create_device(
    ...
    if current_user.role.value not in [ADMIN, ENGINEER]:
        raise HTTPException(
            status_code=403, detail="Only Admin and Engineer can add devices"
        )
    ...
    if existing:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Device ID already exists"
        )
...
async def get_devices(
    ...
    query = db.query(models.Device).options(
        joinedload(models.Device.assignments).joinedload(models.Assignment.license)
    )
...
async def update_device(
    ...
    if current_user.role.value not in [ADMIN, ENGINEER]:
        raise HTTPException(
            status_code=403, detail="Only Admin and Engineer can edit devices"
        )
...
async def create_license(
    ...
    existing = (
        ...
        .filter(models.License.license_key == license_data.license_key)
        .first()
    )
    if existing:
        raise HTTPException(
    ...
    vendor = (
        ...
        .filter(models.Vendor.vendor_id == license_data.vendor_id)
        .first()
    )
    if not vendor:
        raise HTTPException(
...
async def get_license(
    ...
    license = (
        db.query(models.License)
        .filter(models.License.license_key == license_key)
        .first()
    )
    if not license:
...
async def update_license(
    ...
    if "vendor_id" in update_data:
        vendor = (
            db.query(models.Vendor)
            .filter(models.Vendor.vendor_id == update_data["vendor_id"])
            .first()
        )
        if not vendor:
            raise HTTPException(
...
async def create_assignment(
    ...
    if current_user.role.value not in [ADMIN, ENGINEER]:
        raise HTTPException(
            status_code=403, detail="Only Admin and Engineer can assign licenses"
        )
...
@api_router.get(
    "/assignments/by-device/{device_id}", response_model=list[AssignmentResponse]
)
...
async def delete_assignment(
    ...
    if current_user.role.value not in [ADMIN, ENGINEER]:
        raise HTTPException(
            status_code=403, detail="Only Admin and Engineer can unassign licenses"
        )
    ...
    if assignment:
        assignment_info = (
            f"License {assignment.license_key} from device {assignment.device_id}"
        )
    service = AssignmentService(db)
...
async def send_email_alerts(
    ...
    if current_user.role.value not in [ADMIN, ENGINEER]:
        raise HTTPException(
            status_code=403, detail="Only Admin and Engineer can send email alerts"
        )
...
async def send_daily_summary_email(
    ...
    if current_user.role.value != ADMIN:
        raise HTTPException(
            status_code=403, detail="Only Admin can send daily summaries"
        )
    ...
    if not admin_emails:
        return {"success": False, "message": "No admin users found to send summary to"}
    ...
    return {
        "success": success,
        "message": (
            f"Daily summary sent to {len(admin_emails)} admin(s)"
            if success
            else "Failed to send daily summary"
        ),
...
async def get_email_status(current_user: models.User = Depends(get_current_user)):
    ...
    return {
        ...
        "smtp_host": (
            email_service.smtp_host if email_service.enabled else "Not configured"
        ),
        "smtp_user": (
            email_service.smtp_user if email_service.enabled else "Not configured"
        ),
        "message": (
            "Email service is ready"
            if email_service.enabled
            else "Configure SMTP settings in .env to enable email alerts"
        ),
    }
...
async def get_audit_logs(
    ...
    if current_user.role.value not in [ADMIN, AUDITOR]:
        raise HTTPException(
            status_code=403, detail="Only Admin and Auditor can view audit logs"
        )
...
async def root():
    return {
        "message": "License Tracker API",
        "version": "Sprint 3 - Complete with RBAC",
    }
...
async def export_licenses(
    ...
    return StreamingResponse(
        ...
        media_type="text/csv",
        headers={
            "Content-Disposition": f"attachment; filename=licenses_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        },
    )
...
async def export_assignments(
    ...
    return StreamingResponse(
        ...
        media_type="text/csv",
        headers={
            "Content-Disposition": f"attachment; filename=assignments_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        },
    )
...
async def create_software_version(
    ...
    if current_user.role.value not in [ADMIN, ENGINEER]:
        raise HTTPException(
            status_code=403, detail="Only Admin and Engineer can add software versions"
        )

    device = (
        db.query(models.Device)
        .filter(models.Device.device_id == version_data.device_id)
        .first()
    )
    if not device:
        raise HTTPException(status_code=404, detail="Device not found")
    ...
    version = models.SoftwareVersion(
        sv_id=str(uuid.uuid4()), **version_data.model_dump()
    )
    db.add(version)
    db.commit()
    db.refresh(version)
...
@api_router.get(
    "/software-versions/device/{device_id}",
    response_model=list[SoftwareVersionResponse],
)
async def get_device_software_versions(
    ...
    versions = (
        ...
        .filter(models.SoftwareVersion.device_id == device_id)
        .all()
    )
    return versions
...
async def update_software_version(
    ...
    if current_user.role.value not in [ADMIN, ENGINEER]:
        raise HTTPException(
            status_code=403, detail="Only Admin and Engineer can edit software versions"
        )

    version = (
        db.query(models.SoftwareVersion)
        .filter(models.SoftwareVersion.sv_id == sv_id)
        .first()
    )
    if not version:
...
async def delete_software_version(
    ...
    if current_user.role.value != ADMIN:
        raise HTTPException(
            status_code=403, detail="Only Admin can delete software versions"
        )
...
async def get_outdated_count(
    ...
    outdated = (
        db.query(models.SoftwareVersion)
        .filter(models.SoftwareVersion.status == "OUTDATED")
        .count()
    )

    critical = (
        db.query(models.SoftwareVersion)
        .filter(models.SoftwareVersion.status == "CRITICAL")
        .count()
    )

    return {"outdated": outdated, "critical": critical, "total": outdated + critical}
...
async def bulk_upload_devices(
    ...
    if current_user.role.value not in [ADMIN, ENGINEER]:
        raise HTTPException(
            status_code=403, detail="Only Admin and Engineer can bulk upload devices"
        )
    ...
    try:
        ...
        for row_num, row in enumerate(csv_reader, start=2):
            try:
                ...
                existing = (
                    db.query(models.Device)
                    .filter(models.Device.device_id == row["device_id"])
                    .first()
                )
...
async def list_mcp_tools():
    tools = [
        ...
        },
        {
            ...
            "parameters": {
                ...
                "properties": {
                    "status": {
                        "type": "string",
                        "enum": ["ACTIVE", "MAINTENANCE", "DECOMMISSIONED", "all"],
                    },
        ...
        },
        {
            ...
            "parameters": {
                "type": "object",
                "properties": {"days_threshold": {"type": "integer", "default": 15}},
            },
        },
        {
...
async def call_mcp_tool(tool_call: MCPToolCall, db: Session = Depends(get_db)):
    try:
